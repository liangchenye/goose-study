...
func Up_20160530162211(txn *sql.Tx) {
	//	testInitDownNamespace(txn)
	//	testInitDownLayer(txn)

	fmt.Println("up 30 start")
	defer fmt.Println("up end")

	if false {
		testListDownLayer(txn)
		testListDownNamespace(txn)
		if err := Up_20160530162211_Namespace(txn); err != nil {
			fmt.Println(err)
			return
		}
		if err := Up_20160530162211_LayerNamespace(txn); err != nil {
			fmt.Println(err)
			return
		}
	}
...
func Down_20160530162211(txn *sql.Tx) {
	//	testInitUp(txn)
	...
	if false {
		...
		if err := Down_20160530162211_Namespace(txn); err != nil {
			fmt.Println(err)
			return
		}
		if err := Down_20160530162211_LayerNamespace(txn); err != nil {
			fmt.Println(err)
			return
		}
		testListDownLayer(txn)
...
func Up_20160530162211_LayerNamespace(txn *sql.Tx) error {
...
func Down_20160530162211_LayerNamespace(txn *sql.Tx) error {
...
func Up_20160530162211_Namespace(txn *sql.Tx) error {
	rows, err := txn.Query(listDownNamespace)
	if err != nil {
...
func Down_20160530162211_Namespace(txn *sql.Tx) error {
	rows, err := txn.Query(listUpNamespace)
	if err != nil {
		txn.Rollback()
